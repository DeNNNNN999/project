Тема #7 (Node.js): Модуль fs (File System)
...
Неблокирующий I/O: Подавляющее большинство функций модуля fs по умолчанию асинхронны. Они не блокируют Event Loop во время ожидания операции ввода/вывода (чтения с диска, записи на диск).
Колбэки (Старый Стиль): Традиционно, асинхронные fs функции принимали колбэк последним аргументом, который вызывался по завершении операции. Стандартная сигнатура колбэка: (error, data) => { ... }. Если error не null, значит, произошла ошибка.
JavaScript
