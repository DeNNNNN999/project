...
Дискриминирующие Объединения (снова): Проверка общего свойства-дискриминанта (kind, type, status) через === или switch — самый надежный и читаемый способ сужения для таких структур. TS отлично его понимает.
...
Функции Утверждения (Assertion Functions): function assert(condition: any): asserts condition { ... } или function assertIsString(val: any): asserts val is string { ... }. Если функция не выбрасывает ошибку, TS считает, что условие истинно или тип сужен после вызова функции. Используются реже, в основном для функций валидации/утверждения.
