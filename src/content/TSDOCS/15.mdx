...
Исчерпанные Объединения (Union Types): Когда в результате сужения типов (narrowing) не остается ни одного возможного варианта из объединения.
TypeScript
...
Невозможные Пересечения (Intersection Types): Как мы видели, пересечение несовместимых типов дает never.
TypeScript
